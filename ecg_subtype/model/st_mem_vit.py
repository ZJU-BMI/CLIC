...
class ST_MEM_ViT(nn.Module):
    def __init__(self,
        ...
        # classifier head
        if num_classes == 0:
            # if num_classes is 0, we do not use a head
    ...
    def freeze_backbone(self):
        # freeze all parameters in the backbone except for the head
        for name, param in self.named_parameters():
            if 'head' not in name:
                param.requires_grad = False
        print("Backbone parameters are frozen. Only the head is trainable.")
